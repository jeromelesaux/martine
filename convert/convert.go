...
func DowngradingPalette(in *image.NRGBA, size gfx.Size)  *image.NRGBA {
	p,out := downgrade(in)
	fmt.Fprintf(os.Stdout,"Current palette contains (%d) colors\n",len(p))
	if len(p) > size.ColorsAvailable {
		fmt.Fprintf(os.Stderr,"Current palette size (%d) is greater than the available colors in this mode (%d)\n",len(p),size.ColorsAvailable)
		phasis := 1
		for {
