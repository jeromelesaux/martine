...
func ToSprite(in *image.NRGBA,
	...
	size constants.Size,
	mode uint8,
	ex *export.ExportType) (data []byte, firmwareColorUsed map[int]int, lineSize int, err error) {
...
func ToSpriteHard(in *image.NRGBA, p color.Palette, size constants.Size, mode uint8, ex *export.ExportType) (data []byte, firmwareColorUsed map[int]int) {
	size.Height = in.Bounds().Max.Y
	size.Width = in.Bounds().Max.X
	firmwareColorUsed = make(map[int]int)
	offset := 0
