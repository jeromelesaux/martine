...
func Imp(sprites []byte, nbFrames, width, height, mode uint, filename string, export *config.MartineConfig) error {
	...
	impPath := filepath.Join(export.ScreenCfg.OutputPath, export.GetAmsdosFilename(filename, ".IMP"))

	if !export.ScreenCfg.NoAmsdosHeader {
...
func TileMap(data []byte, filename string, export *config.MartineConfig) error {
	...
	impPath := filepath.Join(export.ScreenCfg.OutputPath, export.GetAmsdosFilename(filename, ".TIL"))
